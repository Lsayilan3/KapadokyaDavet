...
namespace Business.Handlers.GalleryTwoes.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetGalleryTwoQuery { GalleryTwoId = request.GalleryTwoId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/gallerytwo");
                    ...
                    result.Data.Photo = "/uploads/gallerytwo/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateGalleryTwoCommand()
                    {
                        GalleryTwoId = result.Data.GalleryTwoId,     
