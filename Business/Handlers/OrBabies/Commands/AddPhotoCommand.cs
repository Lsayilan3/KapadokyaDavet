...
namespace Business.Handlers.OrBabies.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrBabyQuery { OrBabyId = request.OrBabyId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/babies");
                    ...
                    result.Data.Photo = "/uploads/babies/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrBabyCommand()
                    {
                        OrBabyId = result.Data.OrBabyId,
