...
namespace Business.Handlers.OrPikniks.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrPiknikQuery { OrPiknikId = request.OrPiknikId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/picnic");
                    ...
                    result.Data.Photo = "/uploads/picnic/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrPiknikCommand()
                    {
                        OrPiknikId = result.Data.OrPiknikId,
