...
namespace Business.Handlers.OrEkipmans.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrEkipmanQuery { OrEkipmanId = request.OrEkipmanId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/equipment");
                    ...
                    result.Data.Photo = "/uploads/equipment/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrEkipmanCommand()
                    {
                        OrEkipmanId = result.Data.OrEkipmanId,
