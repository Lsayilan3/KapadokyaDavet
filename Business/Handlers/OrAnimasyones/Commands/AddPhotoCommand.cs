...
namespace Business.Handlers.OrAnimasyones.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrAnimasyoneQuery { OrAnimasyoneId = request.OrAnimasyoneId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/animation");
                    ...
                    result.Data.Photo = "/uploads/animation/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrAnimasyoneCommand()
                    {
                        OrAnimasyoneId = result.Data.OrAnimasyoneId,
