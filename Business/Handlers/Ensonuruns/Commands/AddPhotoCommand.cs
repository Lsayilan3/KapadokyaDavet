...
namespace Business.Handlers.Ensonuruns.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetEnsonurunQuery { EnsonurunId = request.EnsonurunId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/latestproduct");
                    ...
                    result.Data.Photo = "/uploads/latestproduct/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateEnsonurunCommand()
                    {
                        Title = result.Data.Title,
                        Url = result.Data.Url,
                        EnsonurunId = result.Data.EnsonurunId,
