...
namespace Business.Handlers.OrPersonelTeminis.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrPersonelTeminiQuery { OrPersonelTeminiId = request.OrPersonelTeminiId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/personnelsupply");
                    ...
                    result.Data.Photo = "/uploads/personnelsupply/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrPersonelTeminiCommand()
                    {
                        OrPersonelTeminiId = result.Data.OrPersonelTeminiId,
