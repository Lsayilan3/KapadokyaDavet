...
namespace Business.Handlers.Organizasyons.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrganizasyonQuery { OrganizasyonId = request.OrganizasyonId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/organization");
                    ...
                    result.Data.Photo = "/uploads/organization/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrganizasyonCommand()
                    {
                        OrganizasyonId = result.Data.OrganizasyonId,
