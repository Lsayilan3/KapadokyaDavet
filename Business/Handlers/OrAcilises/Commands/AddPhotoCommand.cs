...
namespace Business.Handlers.OrAcilises.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrAcilisQuery { OrAcilisId = request.OrAcilisId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/opening");
                    ...
                    result.Data.Photo = "/uploads/opening/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrAcilisCommand()
                    {
                        OrAcilisId = result.Data.OrAcilisId,
