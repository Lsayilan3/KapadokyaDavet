...
namespace Business.Handlers.OrDuguns.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetOrDugunQuery { OrDugunId = request.OrDugunId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/wedding");
                    ...
                    result.Data.Photo = "/uploads/wedding/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateOrDugunCommand()
                    {
                        OrDugunId = result.Data.OrDugunId,
